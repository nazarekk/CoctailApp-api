...
public class PostgresqlLoginDao implements LoginDao {
  ...
  */
  ...
  public boolean getByEmailAndPassword(String email, String password) {
    Collection<Login> query = jdbcTemplate.query(String.format(SELECT_USER, email, password),
        PostgresqlLoginDao::mapUserRow);
    if (query.isEmpty()) {
      return false;
    } else {
      return true;
  ...
  private static Login mapUserRow(ResultSet rs, int rawNum) throws SQLException {
